-- ============================================================================
-- Dataset de entrenamiento para modelo de churn de vendedoras  (v5)
--
-- CAMBIO vs. v4:
--   `HORIZON_CHURN` pasa de 4 a **6 meses**.
--
-- MOTIVACIÓN (ver notebooks/06_horizonte_v4.ipynb para el análisis completo):
--   Replicando el Bloque 2 del NB 01 sobre la granularidad mensual de v4,
--   el codo automático (Δtasa < 2pp entre k consecutivos) cae en k=6, igual
--   que en v3. El cambio de granularidad campaña → mes no movió el codo del
--   horizonte; la elección heurística de k=4 en v4 era prematura.
--
--   Trade-off concreto k=4 → k=6 (sub-muestra fully-observable, ≥3 compras):
--     - churn rate: 32.6% → 27.5% (definición más conservadora)
--     - falsos churn (gaps que vuelven pero serían marcados): 24.5% → 16.1%
--     - Cohen's kappa entre los dos targets: 0.88 (≈95% de etiquetas iguales)
--     - hazard de retorno se aplana entre k=6 (7.3%) y k=7 (5.9%): después
--       de 6 meses sin compra, la probabilidad de retorno es marginal.
--
--   Validación por cohortes (pre-2025 vs post-2025):
--     - En k=6 ambas convergen exactamente (27.5% vs 27.5%).
--     - El régimen post-2025 NO requiere un horizonte más corto — mantener
--       k=4 estaba sobre-ajustado al supuesto de cambio de régimen.
--
-- CAMBIOS PUNTUALES vs. v4:
--
--   1. Definición de churn: vendedora churnea si NO compra en los próximos
--      `HORIZON_CHURN = 6` meses consecutivos. CTE `target` agrega LEAD 5 y
--      6, y la suma de control en `ensamble` se extiende a t1..t6.
--
--   2. CTE `panel`: la cota superior de `mes_rank` pasa de
--      `ultima_compra_rank + 4` a `ultima_compra_rank + 6` para que el
--      último mes observado tenga horizonte de target calculable.
--
--   3. Tabla de salida: `training_churn_v5` (v4 se mantiene intacta para
--      comparación lado-a-lado en NB 07).
--
-- SIN CAMBIOS vs. v4:
--   - Granularidad mensual, agregación, blacklist de campañas.
--   - Filtro `compras_historicas >= 3`.
--   - Schema de features (RFM u3/u6/u12, recencia, tendencias normalizadas).
--   - Mismas exclusiones en el SELECT final.
-- ============================================================================

CREATE OR REPLACE TABLE `glamour-peru-dw.glamour_dw.training_churn_v5`
...
--    El panel cubre desde su primer mes con compra hasta ultimo_mes_compra + 6
...
-- ----------------------------------------------------------------------------
-- 9. Target: churn en los próximos 6 meses (HORIZON_CHURN)
-- ----------------------------------------------------------------------------
target AS (
  SELECT
    id_vendedor,
  ...
  SELECT
    ...
    CASE
      WHEN t.compro_t1 IS NULL
        ...
        THEN NULL
      WHEN (t.compro_t1 + t.compro_t2 + t.compro_t3 + t.compro_t4
          + t.compro_t5 + t.compro_t6) = 0 THEN 1
      ELSE 0
    END AS churn
...
-- ----------------------------------------------------------------------------
-- 11. Resultado final
--     Filtros (equivalentes a v4 pero con horizonte 6):
--       - churn IS NOT NULL: target calculable (6 meses siguientes presentes).
