-- ============================================================================
-- Dataset de entrenamiento para modelo de churn de vendedoras  (v6)
--
-- CAMBIO vs. v5:
--   Filtro temporal post-pandemia: `mes_obs >= '2022-01-01'`.
--
-- MOTIVACIÓN:
--   La pandemia (2020-2021) introdujo shocks operacionales y de demanda
--   que no son representativos del régimen actual de la operación
--   (cuarentenas, suspensiones de catálogos, cambios bruscos en canales
--   de pago). Modelar sobre datos pre-2022 puede transferir patrones que
--   ya no aplican.
--
--   Cutoff 2022-01-01 elegido porque:
--     - Termina la fase de cuarentenas duras en Perú.
--     - Empieza el régimen de "nueva normalidad" sin restricciones.
--     - Mantiene ≈3.8 años de datos (2022-01 → 2025-10) suficientes para
--       entrenar y validar split forward.
--
--   Esta variante mantiene el filtro habitual `compras_historicas >= 3`.
--   La variante hermana `qry_churn_v6_all.sql` lo remueve (todos los
--   vendedores) para comparar el efecto del filtro de población sobre
--   datos limpios de pandemia.
--
-- CAMBIOS PUNTUALES vs. v5:
--
--   1. Filtro final: agrega `AND mes_obs >= DATE '2022-01-01'`.
--   2. Tabla de salida: `training_churn_v6`.
--
-- SIN CAMBIOS vs. v5:
--   - Granularidad mensual, blacklist de campañas, HORIZON_CHURN = 6.
--   - Filtro `compras_historicas >= 3` y `_compro_en_obs = 1`.
--   - Schema de features.
-- ============================================================================

CREATE OR REPLACE TABLE `glamour-peru-dw.glamour_dw.training_churn_v6`
...
WHERE churn IS NOT NULL
  ...
  AND mes_obs >= DATE '2022-01-01'   -- post-pandemia
;
