-- ============================================================================
-- Dataset de entrenamiento para modelo de churn de vendedoras  (v6_all)
--
-- VARIANTE de v6: mismo filtro post-pandemia (`mes_obs >= '2022-01-01'`)
-- pero SIN el filtro de población `compras_historicas >= 3`. Incluye a
-- todos los vendedores con al menos 1 mes de compra observado y horizonte
-- de target calculable.
--
-- MOTIVACIÓN:
--   Probar si, ahora que los datos están limpios de pandemia, el filtro
--   `compras_historicas >= 3` sigue siendo necesario o si vendedores con
--   poca historia (1-2 meses con compra) aportan señal sin saturar el
--   target. En v3 ese filtro era crítico (45% de vendedoras tenía 1 sola
--   compra y el target era trivialmente 1 para ellas), pero el contexto
--   post-pandemia puede haber cambiado la distribución de actividad.
--
--   Hipótesis a contrastar contra v6 (filtro habitual):
--     - Si AUC sube → vendedores nuevos aportan señal valiosa.
--     - Si AUC baja > 1pp → la regla "1 compra → churn" sigue dominando.
--     - Si churn rate sube fuerte → confirmación de que vendedores con
--       poca historia están sesgando el target.
--
-- CAMBIOS PUNTUALES vs. v6:
--
--   1. Se elimina la cláusula `AND compras_historicas >= 3` del WHERE.
--   2. Tabla de salida: `training_churn_v6_all`.
--
-- SIN CAMBIOS vs. v6:
--   - Granularidad mensual, blacklist de campañas, HORIZON_CHURN = 6.
--   - Filtro temporal `mes_obs >= '2022-01-01'`.
--   - Filtro estructural `_compro_en_obs = 1` (solo meses con compra).
--   - Schema de features.
-- ============================================================================

CREATE OR REPLACE TABLE `glamour-peru-dw.glamour_dw.training_churn_v6_all`
...
WHERE churn IS NOT NULL
  ...
  AND mes_obs >= DATE '2022-01-01'   -- post-pandemia (sin filtro de hist)
;
